...
importData()
```
```{r include = F, cache = F, eval = F}
path = "./Rocky_Intertidal/NETN/5_Data/2_DataFiles/Temperature/Compiled_HT_water_temps_2011-2022/"
...
trans_spp_plotly <- plotPISpecies(park = park, years = years, title = F, xlab = NULL, #facet = F,
                      species = 'all', main_groups = T, rev_axis = F, ribbon = T)
```
```{r, warning = FALSE, message = FALSE, results = 'hide', include = FALSE, eval = run}
  ...
  subp <-
     plotly::subplot(
       plotMotileInvertCounts(site = loc, main_groups = TRUE,
                              years = 2013:2018, xaxis = F),
       plotPhotoCover(site = loc, main_groups = T, years = 2013:2018),
...
echino_cnt_plot <- purrr::map(locs$SiteCode,
  ~plotEchinoCounts(park = park, site = ., years = years)) |> 
  set_names(locs$SiteCode)
```
